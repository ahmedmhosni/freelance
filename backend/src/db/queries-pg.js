...
async function createUser(userData) {
  const { name, email, password, role = 'freelancer', email_verified = false, 
          email_verification_token, email_verification_code, email_verification_expires } = userData;
...
async function createProject(projectData) {
  const { user_id, client_id, name, description, status, budget, start_date, end_date } = projectData;
  
  const result = await query(
    `INSERT INTO projects (user_id, client_id, name, description, status, budget, start_date, end_date, created_at)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8, NOW())
     RETURNING *`,
    [user_id, client_id, name, description, status, budget, start_date, end_date]
  );
...
async function createTask(taskData) {
  const { user_id, project_id, title, description, status, priority, due_date, assigned_to } = taskData;
  
  const result = await query(
    `INSERT INTO tasks (user_id, project_id, title, description, status, priority, due_date, assigned_to, created_at)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8, NOW())
     RETURNING *`,
    [user_id, project_id, title, description, status, priority, due_date, assigned_to]
  );
...
async function createInvoice(invoiceData) {
  const { user_id, client_id, project_id, invoice_number, issue_date, due_date, 
          amount, tax, total, status, notes } = invoiceData;
