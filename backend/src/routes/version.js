...
function parseCommitMessage(message) {
  ...
  if (lowerMessage.includes('fix') || lowerMessage.includes('bug')) {
    type = 'fix';
    icon = '🐛';
  } else if (lowerMessage.includes('add') || lowerMessage.includes('new') || lowerMessage.includes('feature')) {
    type = 'feature';
    icon = '✨';
    ...
    type = 'security';
    icon = '🔒';
  } else if (lowerMessage.includes('style') || lowerMessage.includes('design') || lowerMessage.includes('ui')) {
    type = 'design';
    icon = '🎨';
  ...
  try {
    ...
    const allCommits = execSync(
      ...
      .map(line => {
        ...
        // Skip noise commits
        if (
          lowerMessage.match(/^(merge|wip|temp|test|debug|cleanup|minor|typo|formatting)/i) ||
          lowerMessage.match(/^(update readme|update gitignore|update package|update dependencies)/i) ||
        ...
        if (!['feature', 'improvement', 'fix', 'design', 'security'].includes(parsed.type)) {
          return null;
        }
