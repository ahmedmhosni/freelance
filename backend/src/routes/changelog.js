...
router.get('/current-version', async (req, res) => {
  try {
    ...
    res.json({
      ...
      release_date:
        result.rows[0]?.release_date || new Date().toISOString().split('T')[0],
    });
  } catch (error) {
  ...
  try {
    ...
    for (const version of versions) {
      ...
      version.grouped = {
        ...
        improvements: itemsResult.rows.filter(
  ...
  async (req, res) => {
    try {
      ...
      const oldVersion = await pool.query(
        'SELECT version_name, is_major_release FROM versions WHERE id = $1',
        [id]
      );
    ...
    try {
      ...
      if (!fs.existsSync(gitDir)) {
        return res.status(400).json({
          ...
          message:
            'Git sync only works in development environment. In production, create versions manually.',
        });
      }

      // Get last synced commit
      const syncStatus = await pool.query(
        'SELECT last_synced_commit FROM git_sync_status WHERE id = 1'
      );
      const lastCommit = syncStatus.rows[0]?.last_synced_commit;

      // Get new commits since last sync
      let gitCommand =
        'git log --format="%H|%an|%ae|%cd|%s" --date=iso --no-merges';
      if (lastCommit) {
      ...
      try {
        ...
        return res.status(400).json({
          ...
          message:
            'Unable to execute git commands. Make sure git is installed and you are in a git repository.',
        });
      }
      ...
      for (const line of commits) {
        ...
        if (
          lowerMessage.match(
            /^(merge|wip|temp|test|debug|minor|typo|formatting)/i
          ) ||
          message.length < 10
        ) {
          continue;
...
router.get(
  ...
  async (req, res) => {
    ...
    `);
      ...
      res.status(500).json({
        error: 'Failed to fetch version names',
        message: error.message,
      });
    }
  }
