...
const ChangelogEditor = () => {
  ...
  const fetchVersionNames = async () => {
    try {
      const [minorRes, majorRes] = await Promise.all([
        api.get(
          '/api/changelog/admin/version-names?type=minor&unused_only=true'
        ),
        api.get(
          '/api/changelog/admin/version-names?type=major&unused_only=true'
        ),
      ]);
  ...
  const fetchVersionDetails = async (versionId) => {
    try {
      const response = await api.get(
        `/api/changelog/admin/versions/${versionId}`
      );
  ...
  const handleCreateVersion = async (e) => {
    ...
    try {
      if (editingVersionId) {
        await api.put(
          `/api/changelog/admin/versions/${editingVersionId}`,
          versionForm
        );
      } else {
  ...
  const handleAddItem = async (e) => {
    ...
    try {
      if (editingItem) {
        ...
        await api.post(
          `/api/changelog/admin/versions/${selectedVersion.id}/items`,
          itemForm
        );
      }
  ...
  const handleEditVersion = (version) => {
    ...
    const dateOnly = version.release_date
      ? version.release_date.split('T')[0]
  ...
  const handleCreateFromCommits = async (selectedCommits) => {
    try {
      ...
      alert(
        `Version ${nextVersion} created with ${selectedCommits.length} items!`
      );
                ...
                >
                  ...
                  >
                    ...
                    >
                      ...
                      <div>
                        ...
                        >
                          ...
                          >
                            {version.items_count} item
                            {version.items_count !== '1' ? 's' : ''}
                          </span>
                        </div>
                        <div
                          style={{
                    ...
                    <div style={{ display: 'flex', gap: '4px' }}>
                      ...
                      <button
                        onClick={() =>
                          handleTogglePublish(version.id, version.is_published)
                        }
