...
const AdminPanel = () => {
  ...
  const fetchInactiveUsers = async () => {
    try {
      const response = await api.get(
        `/api/admin/activity/inactive-users?days=${inactiveDays}`
      );
  ...
  const handleDeleteInactiveUsers = async (userIds) => {
    if (
      !confirm(
        `Delete ${userIds.length} inactive user(s)? This action can be undone from the GDPR panel.`
      )
    ) {
      return;
    }
  ...
  const handleBulkDeleteInactive = async () => {
    if (
      !confirm(
        `Delete ALL users inactive for ${inactiveDays}+ days? This action can be undone from the GDPR panel.`
      )
    ) {
      return;
    }
    ...
    try {
      await api.post('/api/admin/activity/delete-inactive', {
        days: inactiveDays,
      });
      fetchUsers();
      fetchInactiveUsers();
  ...
  const handleVerificationChange = async (userId, newStatus) => {
    ...
    try {
      await api.put(`/api/admin/users/${userId}/verification`, {
            ...
            >
              <div className="card">
                ...
                <div className="stat-value">
                  {activityStats.userStats.active_7_days}
                </div>
              </div>
              <div className="card">
                <div className="stat-label">ACTIVE (30 DAYS)</div>
                <div className="stat-value">
                  {activityStats.userStats.active_30_days}
                </div>
              </div>
              <div className="card">
                <div className="stat-label">NEVER LOGGED IN</div>
                <div className="stat-value">
                  {activityStats.userStats.never_logged_in}
                </div>
              </div>
              <div className="card">
                <div className="stat-label">INACTIVE (90+ DAYS)</div>
                <div className="stat-value">
                  {activityStats.userStats.inactive_90_days}
                </div>
              </div>
            </div>
          )}
          ...
          <div className="card" style={{ marginBottom: '24px' }}>
            ...
            >
              <div>
                ...
                >
                  Clean up accounts that haven't logged in for a specified
                  period
                </p>
              </div>
              <div
              ...
              <div style={{ marginTop: '16px' }}>
                ...
                >
                  <p style={{ fontSize: '14px', margin: 0 }}>
                    Found <strong>{inactiveUsers.length}</strong> user(s)
                      ...
                      {inactiveUsers.map((user) => (
                        ...
                        >
                          ...
                          <td style={{ padding: '8px' }}>
                            <button
                              onClick={() =>
                                handleDeleteInactiveUsers([user.id])
                              }
                              className="btn-delete"
